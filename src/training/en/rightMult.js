...
module.exports = () => (ctx) => {
    ...
    mongo.connect(url, {
        ...
        db.collection('users').find({ "id": ctx.from.id }).toArray((err, data) => {
            db.collection('users').updateOne({ "id": ctx.from.id }, { $set: { "true_answers" : data[0].true_answers + 1, "last_time_used": new Date().getTime(), "multiplication": data[0].multiplication + 1 } }, (err, result) => {
