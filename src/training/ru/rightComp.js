...
module.exports = () => (ctx) => {
    ...
    mongo.connect(url, {
        ...
        db.collection('users').find({ "id": ctx.from.id }).toArray((err, data) => {
            db.collection('users').updateOne({ "id": ctx.from.id }, { $set: { "true_answers" : data[0].true_answers + 1, "last_time_used": new Date().getTime(), "comparison": data[0].comparison + 1 } }, (err, result) => {
    ...
    mongo.connect(url, {
        ...
        db.collection('users').find({ "id": ctx.from.id }).toArray((err, data) => {
            ...
            let sample = math.getRandomCompare(1, difficulty);
            textToImage.generate(`\n${sample[0].example}`, {
                ...
                if (sample[0].answers[0] === trueAnswer) {
                    answerDataF = 'ru_right_comp';
                } else if (sample[0].answers[1] === trueAnswer) {
                    answerDataS = 'ru_right_comp';
                } else if (sample[0].answers[2] === trueAnswer) {
                    answerDataT = 'ru_right_comp';
                }
